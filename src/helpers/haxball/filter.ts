...
const chatFilter = (message: string) => {
  ...
  if (badWord.includes(message.toLocaleLowerCase())) return true;
  else false;
};

const playerNameFilter = async (room: any, player: any) => {
  const bannedName = ['muhammed', 'allah', 'atatürk', 'ateist', 'deist','agnostik'];

  if (badWord.includes(player.name) || bannedName.includes(player.name)) {

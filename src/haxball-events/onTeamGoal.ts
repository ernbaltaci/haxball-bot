...
const onTeamGoal = (room: any, client: Client) => {
  room.onTeamGoal = (team: any) => {
    ...
    const scoreStuats = score.red > score.blue ? `Kırmızı takım Mavi takımdan ${score.red - score.blue} puan önde.` : `Mavi takım Kırmızı takımdan ${score.blue - score.red} puan önde.`

    sendSuccessToServer(room, undefined, `Takım ${team}, gol attı ve ${scoreStuats}`);
  };
};
